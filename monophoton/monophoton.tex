...
In the following section we're going to deal with all the sources of background that could mimic the monophoton signals and could enter the SR.
...
Four CRs shown in \Fig{\ref{fig:regions}}, enriched in a specific backgrounds process are defined in order to extract normalization factors ($k_Z$, $k_W$, $k_\gammajet$) for the main background, that in the SR.
...
\section{Statistical model, spostami}
